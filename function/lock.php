...
$query=mysql_query("select * from users where id='$id1'") or die("Error");
...
<?php
	if(isset($_POST['unlock']))
	{
		...
		$query=mysql_query("select * from users where password='$password' and id='$id1' ") or die(mysql_error());
		if($num=mysql_num_rows($query)>0)
		{
			$lock="UNACTIVE";
			if (mysql_query("update users set status='$lock' where id='$id1' "))
				{ 
					if($level=="ADMIN")
					{
						header("location:../administrator/myprofile.php?unlocked");
					}
					elseif($level=="INSTRUCTOR")
					{
						header("location:../instructors/myprofile.php?unlocked");
					}
					else
					{
						header("location:lock.php?Access Deneid!");
					}
					
				}
			else
			{
				header("location:lock.php?User does not exist!");
			}
		}
		else
		{
			header("location:lock.php?Incorrect password!");
		}
	}
?>
...
<div class="page-lock">
	...
	{
	?>
		<img class="page-lock-img" src="../administrator/images/adminprofile/<?php echo $image;?>" alt="">
	<?php
	}
	elseif($level=="INSTRUCTOR")
	{
	?>
		<img class="page-lock-img" src="../instructors/images/profilepic/<?php echo $image;?>" alt="">
	<?php
	}
?>	
