...
$id=$_SESSION['timkey'];
	...
	echo "<script type='text/javascript'>
				...
				window.location='../administrator/myprofile.php';
				</script>";
	}
	else
	{
		...
		if(!in_array($ImageExt, $allowedExts))
    {
		...
		{
			$thumb_dir = "../administrator/images/adminprofile/";
			$img_dir = "../administrator/images/adminprofile/";
		...
		if($num=mysql_num_rows($res)>0)
		{
			...
			if(file_exists($thumb_dir. $images . '.jpg'))
			{
				if (unlink($thumb_dir . $images . '.jpg'))
				{
					...
					move_uploaded_file($_FILES["propic"]["tmp_name"],"../administrator/images/adminprofile/" . $_FILES["propic"]["name"]);
